...
void W5500_Process_IP_Frame(uint8_t *frame, uint16_t len) {
    ...
    if (cmd == CMD_SET) {
		...
		uint8_t ip[4] = {0};
		uint8_t dport_buf[2] = {0};
	
		// 소켓 번호는 항상 0번 사용 중이므로 Sn_DIPR(0) = 0x000C, Sn_DPORT(0) = 0x0010
		W5500_ReadBuf(Sn_DIPR(0), 0x0C, ip, 4); 		  // 목적지 IP
		W5500_ReadBuf(Sn_DPORT(0), 0x0C, dport_buf, 2);   // 목적지 포트
	
		uint16_t port = (dport_buf[0] << 8) | dport_buf[1];
	
		uint8_t tx[IP_FRAME_LEN] = {0};
		tx[0] = STX;
		tx[1] = 0x23;
	
		memcpy(&tx[2], ip, 4);		 // DIPR
		memset(&tx[6],	0x00, 4);	 // SUBR = unused
		memset(&tx[10], 0x00, 4);	 // GAR = unused
		tx[14] = port >> 8;
		tx[15] = port & 0xFF;
		tx[16] = calc_checksum(tx, 16);
		tx[17] = ETX;
